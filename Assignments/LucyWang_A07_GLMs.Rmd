...
> Answer: Th figure suggests that the temperature decreases as the depth increases (when the depth is smaller than 10). When depth is smaller than 10, it shows a negative linearity between the two variables. When depth is greater than 10, there is no linearity showing the same linear trend.   
...
> Answer: The final set of explanatory variables that the AIC method suggests we use to predict temperature are year4, daynum, and depth. This model explains 70.44% of the variance, which is an improvement over the previous model using only depth as the explanatory variable. 
...
```{r t.test}
NTL.July.CramptionWard <- NTL.Lake.ChemPhys.July %>%
  filter(lakename %in% c('Crampton Lake', 'Ward Lake'))

CramptonWard.twosample <- t.test(NTL.July.CramptionWard$temperature_C ~ NTL.July.CramptionWard$lakename)
