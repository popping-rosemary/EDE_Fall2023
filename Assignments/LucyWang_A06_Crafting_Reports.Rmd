...
Ozone_2018_boxpot <- ggplot(subset(epa_data, Year = 2018), 
                            aes(x= factor(epa_data$Month, levels=1:12), y=Ozone))+
  geom_boxplot()+
  xlab("Month")+
  ylab("Concentration (µg/m3)")+
  ggtitle("Recorded Ozone by Month (2018)")

print(Ozone_2018_boxpot)
```

```{r boxplot 2019, echo=FALSE, warning = FALSE, fig.height = 3, echo = FALSE, fig.align='left', fig.cap="Montly Ozone, 2019"}

Ozone_2019_boxpot <- ggplot(subset(epa_data, Year = 2019), 
                            aes(x= factor(epa_data$Month, levels=1:12), y=Ozone))+
  geom_boxplot()+
  ylab("Concentration (µg/m3)")+
  xlab("Month")+
