...
```{r DataExercise-1a}
high_temp <- c(91, 83, 83, 84,78, 71, 79, 82, 85, 84)
...
```{r DataExercise-1b}
high_celsius <- round((high_temp - 32) * 5 / 9)
low_celsius <- round((low_temp - 32) * 5/ 9)
```

Combine your four vectors into a data frame and add informative column names.

```{r DataExercise-1c}
df_temp_durham <- data.frame(high_temp, low_temp, high_celsius, low_celsius)
names(df_temp_durham) <- c("Highest temperature (F)", "Lowest temperature (F)", "Highest temperature (C)", "Lowest Temperature (C)"); View(df_temp_durham) 
```
...
```{r DataExercise-1d}
summary(df_temp_durham)
summary(df_temp_durham$`Highest temperature (C)`)
summary(df_temp_durham$`Lowest Temperature (C)`)
sd(df_temp_durham$`Highest temperature (F)`)
sd(df_temp_durham$`Lowest temperature (F)`)
```
