...
Rainfall_BR_processed <-
  ...
  separate(Statistics,c("Null","Month","Null2")," ") %>% # separate the statistics column 
  mutate( Date = my(paste0(Month,"-",Year))) %>% # month-year
...
# create a sequence, equally distributed by month, join the sequence of date with the columns 
...
ggplot(BR_complete, aes(x = Date, y = Rainfall))+ 
  geom_line()+
  geom_smooth()
# seasonality presents 

ggplot(BR_complete, aes(x=Date, y = Temperature_C))+ 
  geom_line()+
  geom_smooth()
# temperature has an upward trend 
