...
```{r}
#Exercise 1:
OzonebyYear_density <- ggplot(EPAair, aes(x = Ozone, 
                                          fill=as.factor(Year)))+
  geom_density(alpha = 0.5)+
  scale_fill_manual(values = c('darkgreen', 'darkblue'))+
  labs(x='Ozone AQI value', y='Density', fill = '')
print(OzonebyYear_density)

OzonebyYear_boxplot <- ggplot(EPAair, aes(x = as.factor(Year), y=Ozone))+
  geom_boxplot()+
...
print(OzonebyYear_boxplot)

OzonebyYear_violin <- ggplot(EPAair, aes(x = as.factor(Year), y=Ozone))+
...
corrplot(EPAairCorr, meghod = 'ellipse')
...
OzonebyAll.regression <- lm(data = EPAair.subset, PM2.5 ~ Ozone + Year + Month + mean_log + mean_Lat)
